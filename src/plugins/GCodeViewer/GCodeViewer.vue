<template>
  <div ref="primarycontainer" v-resize="resize" class="primary-container mt-2">
    <div :class="{ 'full-screen': fullscreen }" class="viewer-box">
      <div v-show="fullscreen" :class="emergencyButtonClass">
        <code-btn
          :code="'M112\nM999'"
          :log="false"
        ...
        <v-expansion-panels>
          <v-expansion-panel @click="scrollIntoView">
            ...
            <v-expansion-panel-content eager>
              <v-btn-toggle v-model="renderQuality" block class="btn-toggle d-flex" exclusive>
                <v-btn :disabled="loading" :value="1" block>
                  {{ $t('plugins.gcodeViewer.sbc') }}
                </v-btn>
                <v-btn :disabled="loading" :value="2" block>
                  {{ $t('plugins.gcodeViewer.low') }}
                </v-btn>
                <v-btn :disabled="loading" :value="3" block>
                  {{ $t('plugins.gcodeViewer.medium') }}
                </v-btn>
                <v-btn :disabled="loading" :value="4" block>
                  {{ $t('plugins.gcodeViewer.high') }}
                </v-btn>
                <v-btn :disabled="loading" :value="5" block>
                  {{ $t('plugins.gcodeViewer.ultra') }}
                </v-btn>
                <v-btn :disabled="loading" :value="6" block>
          ...
          <v-expansion-panel @click="scrollIntoView">
            ...
            <v-expansion-panel-content>
              <v-card>
                ...
                <v-btn-toggle v-model="colorMode" block class="btn-toggle d-flex" exclusive>
                  <v-btn :disabled="loading" :value="0" block>
                ...
                <v-slider v-model="minColorRate" :max="500" :min="5" thumb-label />
                <h4>{{ $t('plugins.gcodeViewer.maxFeedrate') }}</h4>
                <v-slider v-model="maxColorRate" :max="500" :min="5" thumb-label />
              </v-card>
              <v-card>
                <v-card-title>
