...
export default class {
  ...
  build() {
    ...
    let g = this.svg.append('g').attr('transform', 'translate(50,40)')
    ...
    for (let idx = 0; idx <= 10; idx++) {
      g.append('rect')
        ...
        .attr(
          'transform',
          `rotate(${this.arcStartPoint + tickDistance * idx}) translate(0,-34)  scale(1,1)`,
        )
    }

    this.svgSetTemp = g.append('g')
    this.svgSetTempElement = this.svgSetTemp
      .append('rect')
      .attr('width', '2px')
      .attr('height', '4px')
      .attr('x', '-1px')
  ...
  update(currentValue) {
    ...
    this.svgSetTemp.attr('transform', `rotate(${this.getRotationAngle(this.setTemperature)})`)
  }
  ...
  updateState(state) {
    ...
    switch (state) {
      ...
      case 'standby':
        {
          if (
            this.standbyTemperature > this.currentValue - 5 &&
            this.standbyTemperature < this.currentValue + 5
          ) {
      ...
      case 'active':
        if (
          this.setTemperature > this.currentValue - 5 &&
          this.setTemperature < this.currentValue + 5
        ) {
          stateCSS = 'gauge-activeattemp'
