<template>
  <v-row>
          ...
          <v-tab-item value="analysis">
            <div class="d-flex flex-column">
              ...
              <v-alert :value="files.length === 0 && !filesError" type="info" class="mb-0">
                No Motion Profiles found. Record a new Motion Profile or
                <a
              ...
              <v-row v-show="files.length > 0" class="content pa-2">
                ...
                <v-col
                  :class="!!filesToAnalyze.length ? 'd-none' : 'd-flex'"
                  class="align-center justify-center"
                >
                  Please select a Motion Profile
                </v-col>
                <v-col
                  v-show="!!filesToAnalyze.length"
                  :class="!!filesToAnalyze.length ? 'd-flex' : 'd-none'"
      ...
      <v-card tile>
        ...
        <v-card-text class="d-flex flex-column">
          <input-shaper-checkbox
            v-model="inputShapers"
            value="none"
            :current="shaping.type"
            class="mt-0"
          />
          ...
          <input-shaper-checkbox v-model="inputShapers" value="custom" :current="shaping.type">
            ...
            >
              ...
              <v-card>
                <v-card-title> Custom Shaper Configuration </v-card-title>
                <v-card-text class="pb-2">
                  <v-select
                    v-model="numCustomCoefficients"
                    label="Number of Impulses"
                    :items="[0, 1, 2, 3, 4]"
                    hide-details
                  />
                </v-card-text>
                    ...
                    <tr v-for="(_, index) in customAmplitudes" :key="index">
                      ...
                      <td>
                        <v-text-field
                          ...
                          class="pt-0 mb-1"
                          hide-details
                          @input="setCustomAmplitude(index, $event)"
                        />
                      </td>
                      <td>
                        <v-text-field
                          ...
                          class="pt-0 mb-1"
                          hide-details
                          @input="setCustomDuration(index, $event)"
                        />
                      </td>
                    </tr>
                  </tbody>
                ...
                <v-card-text v-show="!!customShaperCode" class="pb-0">
                  ...
                  <div class="d-flex">
                    <input
                      ref="customShaperCode"
                      type="text"
                      :value="customShaperCode"
                      class="flex-grow-1"
                      readonly
                ...
                <v-card-actions class="justify-center">
                  <v-btn
                    text
                    :disabled="!canConfigureCustom"
                    :loading="configuringCustomShaper"
                    color="primary"
          ...
          >
            ...
            <template #append-outer>
              <v-icon class="ml-1" :disabled="!canSetFrequency" @click="setFrequency">
                mdi-check
              </v-icon>
            </template>
          </v-text-field>
          ...
          >
            <template #append-outer>
              <v-icon class="ml-1" :disabled="!canSetDamping" @click="setDamping">
                mdi-check
              </v-icon>
            </template>
          </v-text-field>
    ...
    <record-motion-profile-dialog
      :last-run="lastRun"
      :shown.sync="showDataCollection"
...
export default {
  ...
  computed: {
    ...
    isInputShapingEnabled() {
      return this.shaping.type !== InputShapingType.none
    },
    currentFrequencies() {
      return Array.from({ length: 81 }, (_, index) => index + 10)
    },
    ...
    numCustomCoefficients: {
      get() {
        return this.customAmplitudes.length
      },
      set(value) {
    ...
    canConfigureCustom() {
      return (
        this.numCustomCoefficients > 0 &&
        this.customAmplitudes.every((amplitude) => amplitude > 0) &&
        this.customDelays.every((duration) => duration > 0)
      )
    },
    customShaperCode() {
      if (this.inputShapers.includes('custom') && this.canConfigureCustom) {
        const amplitudes = this.customAmplitudes
          .map((amplitude) => amplitude.toFixed(3))
          .reduce((a, b) => a + ':' + b)
        const delays = this.customDelays
          .map((duration) => duration.toFixed(4))
          .reduce((a, b) => a + ':' + b)
  ...
  methods: {
    ...
    async refresh() {
      ...
      try {
        const files = (await this.getFileList(Path.accelerometer)).filter(
          (file) =>
            !file.isDirectory && file.name !== Path.filamentsFile && file.name.endsWith('.csv'),
        )
    ...
    filesOrDirectoriesChanged({ machine, files, volume }) {
      if (machine === this.selectedMachine && files !== undefined) {
        if (
          this.filesToAnalyze.some((fileToAnalyze) =>
            files.includes(Path.combine(Path.accelerometer, fileToAnalyze)),
          )
        ) {
          // Current file being analyzed has bene changed, invalidate it
          this.filesToAnalyze = []
        } else if (
          files.some((file) => file.endsWith('.csv')) &&
          Path.filesAffectDirectory(files, Path.accelerometer)
        ) {
