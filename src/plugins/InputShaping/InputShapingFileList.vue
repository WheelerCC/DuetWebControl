<template>
  <v-card flat class="d-flex flex-column">
    ...
    <v-card-text v-show="files.length === 0" class="pa-0">
      <v-alert :value="true" type="info" class="mb-0"> No Profiles </v-alert>
    </v-card-text>
    ...
    <template v-else>
      <v-alert type="info" class="mb-0"> No Profiles found </v-alert>
    </template>
...
export default {
    ...
    profiles() {
      ...
      for (let i = 0; i < this.files.length; i++) {
        const filename = this.files[i],
          lastModified = this.filesLastModified[i].toLocaleString()
        const matches =
          /^(\d+)-([a-zA-SU-Z]+)(-?\d+\.?\d*)-(-?\d+\.?\d*)-(\d+\.?\d*)-(\w+)-?(\d+\.?\d*)?(Hz)?(-(\d+\.?\d*))?\.csv/.exec(
            filename,
          )
        if (matches) {
          ...
          const toolMatches =
            /^(\d+)-T(\d+)-([a-zA-Z]+)(-?\d+\.?\d*)-(-?\d+\.?\d*)-(\d+\.?\d*)-(\w+)[-]?(\d+\.?\d*)?(Hz)?(-(\d+\.?\d*))?\.csv/.exec(
              filename,
            )
          if (toolMatches) {
            ...
            const shaperTitle =
              toolMatches[7] === 'none'
                ? 'No Shaping'
                : toolMatches[7] === 'custom'
                  ? 'Custom'
                  : `${toolMatches[7].toUpperCase()} @ ${toolMatches[8]}Hz`
            const dampingFactor = isNaN(toolMatches[11])
              ? null
              : `Damping Factor ${toolMatches[11]}`
            run.files.push({
  ...
  methods: {
    ...
    async getSamples(filename) {
      ...
      // Extract details
      const details = /Rate (\d+) overflows (\d)/.exec(
        csv.content[csv.content.length - 1].reduce((a, b) => a + b),
      )
      if (!details) {
    ...
    getFrequencyResponse(datasets) {
      ...
      const samplingRate =
        datasets.length > 1
          ? datasets.map((dataset) => dataset.samplingRate).reduce((a, b) => a + b) /
            datasets.length
          : datasets[0].samplingRate
      const numAxes = datasets[0].samples.length,
        numSamples = datasets[0].samples[0].length
      ...
      const firstDataResult = analyzeAccelerometerData(
        datasets[0].samples,
        samplingRate,
        this.wideBand,
      )
      const frequencies = firstDataResult.frequencies,
        amplitudes = firstDataResult.amplitudes
      for (let dataset = 1; dataset < datasets.length; dataset++) {
        const dataResult = analyzeAccelerometerData(
      ...
      return {
        frequencies,
        amplitudes: amplitudes.map((axisValues) =>
          axisValues.map((amplitude) => amplitude / datasets.length),
        ),
      }
    },
    async update() {
      ...
      if (this.selection.length > 0) {
        ...
        if (this.individualFiles && this.showSamples) {
          ...
          try {
            ...
            if (
              datasets.length === 1 &&
              typeof this.sampleStartIndex === 'number' &&
              typeof this.sampleEndIndex === 'number'
            ) {
