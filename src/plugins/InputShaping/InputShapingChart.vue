<template>
  <canvas ref="chart" @mousedown="mouseDown" @mousemove="mouseMove" @dblclick="doubleClick" />
</template>
...
export default {
  props: {
    ...
    showValues: {
      // whether the values are supposed to be shown as datasets
      default: true,
      type: Boolean,
    },
    sampleStartIndex: {
  ...
  computed: {
    ...
    showReduction() {
      return (
        ((this.amplitudes && this.delays) ||
          (!!this.inputShapers && this.inputShapers.length > 0)) &&
        !this.estimateShaperEffect
      )
    },
    resolution() {
      return this.frequencies && this.frequencies.length > 2
        ? this.frequencies[1] - this.frequencies[0]
        : 0
    },
    lineAtPoint() {
      let point = -1
  ...
  watch: {
      ...
      handler() {
        if (this.customAmplitudes && this.customDelays) {
          for (let dataset in this.chart.data.datasets) {
            if (dataset.isCustom) {
              dataset.data = getInputShaperDamping(
                this.frequencies,
    ...
    sampleEndIndex(to) {
      if (!this.frequencies || this.frequencies.length === 0) {
        this.chart.config.options.scales.xAxes[0].ticks.max = isNaN(to)
          ? this.chart.data.datasets.labels.length
          : to
        this.chart.update()
      }
    },
        ...
        xAxes: [
          {
            ...
            scaleLabel: {
              display: true,
              labelString:
                this.frequencies && this.frequencies.length > 0 ? 'Frequency (in Hz)' : 'Sample',
            },
            ticks: {
              minor: {
      ...
      tooltips: {
        ...
        callbacks: {
          label(tooltipItem, data) {
            ...
            label += that.estimateShaperEffect
              ? Math.round(tooltipItem.yLabel * 10000) / 10000
              : Math.round(tooltipItem.yLabel * 1000) / 1000
            return label
          },
          title: (items) =>
            that.frequencies
              ? that.$t('plugins.accelerometer.frequencyTooltip', [
                  that.frequencies[items[0].index].toFixed(1),
                  (that.resolution / 2).toFixed(1),
                ])
  ...
  methods: {
    ...
    updateDatasets() {
      const hiddenDatasets = this.chart.data.datasets
        .filter((_, index) => !this.chart.isDatasetVisible(index))
      ...
      this.chart.data.labels =
        this.frequencies && this.frequencies.length > 0
          ? this.frequencies.map((freq) => Math.round(freq).toString())
      ...
      if (this.inputShaperFrequency && this.frequencies && this.frequencies.length > 0) {
        for (let shaperType of this.inputShapers) {
          ...
          const factors = getInputShaperFactors(
            shaperType,
            this.inputShaperFrequency,
            this.inputShaperDamping,
          )
          const damping = getInputShaperDamping(
            this.frequencies,
            factors.amplitudes,
            factors.durations,
          )
      ...
      if (
        this.inputShapers.includes('custom') &&
        this.frequencies &&
        this.frequencies.length > 0 &&
        this.customAmplitudes &&
        this.customDelays &&
        this.customAmplitudes.length > 0 &&
        this.customDelays.length > 0
      ) {
        const damping = getInputShaperDamping(
          this.frequencies,
          this.customAmplitudes,
      ...
      if (
        !this.chart.data.datasets.find((dataset) => dataset.isShaperFrequency) &&
        this.lineAtPoint !== -1 &&
      ...
      if (this.chart.config.range && this.chart.config.range.end) {
        ...
        if (activePoints && activePoints.length > 0) {
          ...
          if (Math.abs(dragEnd - this.dragStart) > 4) {
            const trueStart = Math.min(this.dragStart, dragEnd),
              trueEnd = Math.max(this.dragStart, dragEnd)
