  ...
  <v-row>
    <v-col cols="12" lg="auto" order="1" order-lg="0" sm="6">
      <v-card tile>
        ...
        <v-card-text v-show="files.length === 0" class="pa-0">
    ...
    <v-col
      ...
      order-lg="0"
    >
      <div ref="container" v-resize="resize" class="heightmap-container">
        ...
        <div class="canvas-container">
          <!-- v-show="ready" -->
          <canvas ref="canvas" @mousemove="canvasMouseMove" />
    ...
    <v-col class="d-flex flex-column" cols="12" lg="auto" order="2" sm="6">
      <v-card class="d-flex flex-column flex-grow-0" tile>
        ...
        <v-card-text class="d-flex flex-column flex-grow-0 justify-space-between pt-2">
          ...
          <span>{{
            $t('plugins.heightmap.maxDeviations', [
              $display(minDiff, 3),
        ...
        <v-card-text class="d-flex flex-column">
          <div class="d-flex flex-column mt-1">
            ...
            <v-btn-toggle v-model="colorScheme" class="mt-1">
              <v-btn class="flex-grow-1" value="terrain">
                {{ $t('plugins.heightmap.terrain') }}
              </v-btn>
              <v-btn class="flex-grow-1" value="heat">
          ...
          <div class="d-flex flex-column mt-1">
            ...
            <v-btn-toggle v-model="deviationColoring" class="mt-1">
              <v-btn class="flex-grow-1" value="fixed">
                {{ $t('plugins.heightmap.fixed') }}
              </v-btn>
              <v-btn class="flex-grow-1" value="deviation">
    ...
    <v-tooltip v-model="tooltip.shown" :position-x="tooltip.x" :position-y="tooltip.y" absolute top>
...
export default {
  ...
  methods: {
    ...
    resize() {
      ...
      let globalContainerHeight = this.$vuetify.breakpoint.smAndDown
        ? 0
    ...
    showHeightMap(points, probeRadius) {
      ...
      this.area = probeRadius
        ? probeRadius * probeRadius * Math.PI
        : Math.abs((xMax - xMin) * (yMax - yMin))
      this.rmsError =
        Math.sqrt(this.rmsError * this.numPoints - this.meanError * this.meanError) / this.numPoints
      this.meanError = this.meanError / this.numPoints
      heightMapViewer.renderHeightMap(
        points,
        this.invertZ,
        this.colorScheme,
        this.deviationColoring,
      )
      heightMapViewer.drawLegend(
        this.$refs.legend,
        this.colorScheme,
    ...
    async refresh() {
      ...
      if (this.files.indexOf(this.selectedFile) === -1) {
        if (
          this.heightmapFile &&
          this.files.indexOf(Path.extractFileName(this.heightmapFile)) !== -1
        ) {
    ...
    filesOrDirectoriesChanged({ machine, files, volume }) {
      if (machine === this.selectedMachine && files !== undefined) {
        if (
          this.selectedFile &&
          files.indexOf(Path.combine(this.systemDirectory, this.selectedFile)) >= 0
        ) {
          ...
          Path.filesAffectDirectory(files, this.systemDirectory)
        ) {
