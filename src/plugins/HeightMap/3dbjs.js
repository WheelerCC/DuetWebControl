...
export default class {
  ...
  renderHeightMap(bedPoints, invertZ, colorScheme = 'terrain', deviationColor = 'fixed') {
    ...
    if (deviationColor === 'deviation') {
      this.maxVisualizationZ =
        Math.abs(this.maxZ) > Math.abs(this.minZ) ? Math.abs(this.maxZ) : Math.abs(this.minZ)
    } else {
      this.maxVisualizationZ = 0.25
    }
  ...
  resetCamera() {
    ...
    if (this.isDelta) {
      ...
      this.scene.activeCamera.setPosition(
        new Vector3(bedCenter.x, this.buildVolume.z.max, this.buildVolume.y.min - bedSize.y / 2),
      )
    } else {
  ...
  renderBed() {
    ...
    if (this.isDelta) {
      ...
      this.bedMesh = MeshBuilder.CreateDisc(
        'BuildPlate',
        { radius: radius, sideOrientation: Mesh.DoubleSide },
        this.scene,
      )
      ...
      let width = bedSize.x
      let depth = bedSize.y
      this.bedMesh = MeshBuilder.CreatePlane(
        'BuildPlate',
        { width: width, height: depth, sideOrientation: Mesh.DoubleSide },
        this.scene,
      )
      this.bedMesh.material = this.gridMaterial
      this.bedMesh.rotationQuaternion = Quaternion.RotationAxis(new Vector3(1, 0, 0), Math.PI / 2)
  ...
  drawLegend(canvas, colorScheme, invertZ, xLabel, yLabel) {
    ...
    context.fillText(
      `${invertZ ? -this.maxVisualizationZ : this.maxVisualizationZ} mm`,
      canvas.width / 2,
      44,
    )
    context.fillText(
      i18n.t(invertZ ? 'plugins.heightmap.orLess' : 'plugins.heightmap.orMore'),
      canvas.width / 2,
      60,
    )
    ...
    if (colorScheme === 'terrain') {
      ...
      context.fillText(
        i18n.t(invertZ ? 'plugins.heightmap.orMore' : 'plugins.heightmap.orLess'),
        canvas.width / 2,
        scaleHeight + 98,
      )
      scaleHeight += 16
