    ...
    for (let key in b) {
      if (skipNonexistentFields && !(key in a)) {
        if (process.env.NODE_ENV !== 'production') {
          console.warn(
            `[patch] Skipped merge of ${fullPath}/${key} because it does not exist in the source. Value: ${JSON.stringify(b[key])}`,
          )
        }
      } else if (b[key] === null) {
        Vue.set(a, key, null)
        ...
        if (
          ...
          if (process.env.NODE_ENV !== 'production') {
            console.warn(
              `[patch] Skipped merge of ${fullPath}/${key} due to incompatible types ${typeof a[key]} vs ${typeof b[key]}. Value: ${JSON.stringify(b[key])}`,
            )
          }
          ...
          patch(
            a[key],
            b[key],
            skipNonexistentFields && !(a[key] instanceof Dictionary),
            fullPath + '/' + key,
          )
