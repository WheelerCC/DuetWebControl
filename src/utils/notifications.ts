...
export function makeNotification(
  ...
  if (timeout === null) {
    timeout =
      type === 'error' && settingsStore.notifications.errorsPersistent
        ? 0
        : settingsStore.notifications.timeout
  }
  ...
  const equalNotification = notifications.find(
    (item) =>
      item.type === type &&
      item.title === title &&
 ...
 */
export function makeFileTransferNotification(
  type: NotificationType,
  filename: string,
