...
import ObjectModel, { Axis, AxisLetter, Board, Extruder, Fan, Heat, Heater, initCollection, initObject, MachineStatus, ModelDictionary, Move, Network, Plugin, Probe, Sensors, State, Tool } from "@duet3d/objectmodel";
import Vue from "vue";
...
export const useMachinesModelStore = defineStore({
	...
	actions: {
		...
		currentTool() {
			...
			if (this[machineName].state.currentTool >= 0 && this[machineName].state.currentTool < this[machineName].tools.length) {
		...
		fractionPrinted() {
			let machineName = useRootStore().selectedMachine
			if (this[machineName].job.file && this[machineName].job.filePosition !== null && this[machineName].job.file.size > 0) {
				return (this[machineName].job.filePosition as number) / (this[machineName].job.file.size as number);
			}
			return 0;
		},
		...
		jobProgress(getters) {
			...
			if (isPrinting(this[machineName].state.status)) {
				if (!isPaused(this[machineName].state.status) && this[machineName].state.status !== MachineStatus.simulating && this[machineName].move.extruders.length > 0 && this[machineName].job.file !== null && this[machineName].job.file.filament.length > 0) {
			...
			for (const key of Object.keys(data)) {
				if (key === "global") {
					...
					if (data.job?.file && data.job.file.customInfo !== undefined) {
						(this[useRootStore().selectedMachine] as WritableDeep<ObjectModel>).job!.file!.customInfo = data.job.file.customInfo;
					}
				} else if (key === "plugins") {
