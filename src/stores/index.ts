...
export const useRootStore = defineStore({
	...
	getters: {
		...
		isConnected(): boolean {
			...
			return this.selectedMachine !== defaultMachine && !(machineStore[this.selectedMachine].isReconnecting);
		},
		uiFrozen(): boolean {
		...
		async connect({ hostname = location.host, username = defaultUsername, password = defaultPassword, retrying = false } = {}) {
			...
			try {
				...
				if (isPasswordError) {
					...
					setTimeout(() => useRootStore().connect({ hostname, username, password, retrying: true }), 1000);
					return;
				}
			}
