...
export const useMachinesCacheStore = defineStore({
	...
	actions: {
		...
		addLastSentCode(code: string) {
			let machineName = useRootStore().selectedMachine
			this[machineName].lastSentCodes = this[machineName].lastSentCodes.filter(item => item !== code);
		...
		setSorting({ table, column, descending }: { table: string, column: string, descending: boolean }) {
			this.sorting[table].column = column;
			this.sorting[table].descending = descending;
		},

		registerPluginData({ plugin, key, defaultValue }: { plugin: string, key: string, defaultValue: any }, _machineName?: string) {
