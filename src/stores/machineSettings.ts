...
export const useMachinesSettingsStore = defineStore({
	...
	getters: {
		...
		fileTransferRetryThreshold: (state) => state[useRootStore().selectedMachine].fileTransferRetryThreshold,
		...
		displayedExtraTemperatures: (state) => state[useRootStore().selectedMachine].displayedExtraTemperatures,
	...
	actions: {
		getMoveSteps(axis: AxisLetter): number[] {
			...
			return (this[machineName].moveSteps[axis] !== undefined) ? this[machineName].moveSteps[axis] : this[machineName].moveSteps.default;
		},
		numMoveSteps(): number {
			let machineName = useRootStore().selectedMachine
		...
		toggleExtraVisibility(sensor: number) {
			...
			if (this[machineName].displayedExtraTemperatures.indexOf(sensor) === -1) {
				...
				this[machineName].displayedExtraTemperatures = this[machineName].displayedExtraTemperatures.filter(heater => heater !== sensor);
			}
		},
		toggleExtruderVisibility(extruder: number) {
			...
			if (this[machineName].displayedExtruders.indexOf(extruder) === -1) {
				...
				this[machineName].displayedExtruders = this[machineName].displayedExtruders.filter(item => item !== extruder);
			}
		},
		toggleFanVisibility(fan: number) {
			...
			if (this[machineName].displayedFans.indexOf(fan) === -1) {
				...
				this[machineName].displayedFans = this[machineName].displayedFans.filter(item => item !== fan);
			}
		},
		mutLoad(payload) {
			let machineName = useRootStore().selectedMachine
			...
			if (payload.moveSteps !== undefined) {
				for (let axis in payload.moveSteps) {
					...
					if (axisMoveSteps instanceof Array && axisMoveSteps.length === this[machineName].moveSteps.default.length) {
		...
		disableDwcPlugin(plugin: string) {
			let machineName = useRootStore().selectedMachine
			this[machineName].enabledPlugins = this[machineName].enabledPlugins.filter(item => item !== plugin);
		},
