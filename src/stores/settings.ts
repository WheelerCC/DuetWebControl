	...
	actions: {
		...
		async reset() {
			...
			try {
				...
				const defaults = await useMachinesStore().download({ filename: Path.dwcFactoryDefaults, showProgress: false, showSuccess: false, showError: false });
		...
		registerPluginData({ plugin, key, defaultValue }: { plugin: string, key: string, defaultValue: any }) {
