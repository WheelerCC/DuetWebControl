...
import { CancellationToken, OperationFailedError, FileNotFoundError, OperationCancelledError, OnProgressCallback, BaseConnector, InvalidPasswordError } from "@duet3d/connectors";
...
import ObjectModel, { DefaultHostname, GCodeFileInfo, MachineStatus, MessageType } from "@duet3d/objectmodel";
...
export const useMachinesStore = defineStore({
	...
	getters: {
		 ...
		 */
		hasTemperaturesToDisplay: (state) => useMachinesModelStore().sensors.analog.some(function (sensor, sensorIndex) {
			const settings = useMachinesSettingsStore()
			return useMachinesModelStore().heat.heaters.some(heater => heater && heater.sensor === sensorIndex) ||
	...
	actions: {
		 ...
		 */
		async setSbcPluginData({ plugin, key, value } : { plugin: string, key: string, value: any }): Promise<void> {
			console.log('todo')
		...
		async getFileList(directory: string) {
			if (this[useRootStore().selectedMachine]._connector === null) { throw new OperationFailedError("getFileList is not available in default machine module"); }
		 ...
		 */
		async getFileInfo({ filename, readThumbnailContent }: { filename: string, readThumbnailContent?: boolean }) {
			if (this.connector === null) { throw new OperationFailedError("getFileInfo is not available in default machine module"); }
		...
		async makeDirectory(_, directory: string) {
			if (this.connector === null) { throw new OperationFailedError("delete is not available in default machine module"); }

			await this.connector.makeDirectory(directory);
			Root.$emit(Events.directoryCreated, { machine: this.connector.hostname, directory });
			Root.$emit(Events.filesOrDirectoriesChanged, { machine: this.connector.hostname, files: [directory] });
		},
		/**
		 * 
		 * @param context Action context
		...
		async move({ from, to, force = false }: { from: string, to: string, force?: boolean }) {
			if (this.connector === null) { throw new OperationFailedError("move is not available in default machine module"); }
			...
			Root.$emit(Events.filesOrDirectoriesChanged, { machine: this.connector.hostname, files: [from, to] });
		},
		 ...
		 */
		async reconnect() {
			if (this.connector === null) { throw new OperationFailedError("reconnect is not available in default machine module"); }
		...
		async onConnectionError(error: Error) {
			if (this.connector === null) { throw new OperationFailedError("onConnectionError is not available in default machine module"); }
			console.warn(error);

			if (!this.isReconnecting && [MachineStatus.updating, MachineStatus.halted].includes(useMachinesModelStore().state.status)) {
				...
				this.reconnect()
			} else if (useMachinesModelStore().isReconnecting && !(error instanceof InvalidPasswordError)) {
		...
		async update(payload: Partial<ObjectModel>) {
			...
			const lastBeepFrequency = machineModelState.state.beep ? machineModelState.state.beep.frequency : null;
			const lastBeepDuration = machineModelState.state.beep ? machineModelState.state.beep.duration : null;
			const lastDisplayMessage = machineModelState.state.displayMessage, lastStatus = machineModelState.state.status;
			const lastDsfVersion = machineModelState.sbc?.dsf?.version;
			const lastStartupError = machineModelState.state.startupError ? JSON.stringify(machineModelState.state.startupError) : null;

			// Check if the job has finished and if so, clear the file cache
			if (payload.job && payload.job.lastFileName && payload.job.lastFileName !== machineModelState.job.lastFileName) {
			...
			if (machineModelState.state.beep &&
			...
			if (lastStatus !== machineModelState.state.status && machineModelState.state.status === MachineStatus.halted) {
				log(LogType.warning, i18n.t("events.emergencyStop"), undefined, this.connector?.hostname ?? DefaultHostname);
			}
		},
		/**
		...
		async disconnect(): Promise<void> {
			...
			if (this.connector === null) { throw new OperationFailedError("disconnect is not available in default machine module"); }
		 ...
		 */
		async installSystemPackage({ filename, packageData, cancellationToken, onProgress } : { filename: string, packageData: Blob, cancellationToken?: CancellationToken, onProgress?: OnProgressCallback}): Promise<void> {
			console.log('todo')
		 ...
		 */
		async sendCode(payload: string | { code: string, fromInput?: boolean, log?: boolean, noWait?: boolean }) {
		 ...
		 */
		async upload(payload: { filename?: string, content?: any, files?: Array<{ filename: string, content: any }>, showProgress?: boolean, showSuccess?: boolean, showError?: boolean, closeProgressOnSuccess?: boolean }) {
			...
			try {
				for (let i = 0; i < files.length; i++) {
					...
					try {
						...
						const configFile = Path.combine(machinesModelStore[machineName].directories.system, Path.configFile);
						if (Path.equals(filename, configFile)) {
							const configFileBackup = Path.combine(machinesModelStore[machineName].directories.system, Path.configBackupFile);
							try {
								console.log('todo connector')
						...
						if (payload.filename && showSuccess) {
							...
							log(LogType.success, i18n.t("notification.upload.success", [Path.extractFileName(filename), displayTime(secondsPassed)]), undefined, machineName);
						}
						...
						if (showError && !(e instanceof OperationCancelledError)) {
							console.warn(e);
							log(LogType.error, i18n.t("notification.upload.error", [Path.extractFileName(filename)]), getErrorMessage(e), machineName);
						}
		 ...
		 */
		async download(payload: { filename?: string, type?: XMLHttpRequestResponseType, files?: Array<string>, showProgress?: boolean, showSuccess?: boolean, showError?: boolean, closeProgressOnSuccess?: boolean, rawPath?: boolean }): Promise<FileTransferItem | Array<FileTransferItem>> {
			console.log('todo')
			return [];
		...
		async loadDwcPlugin({ id, saveSettings }: { id: string, saveSettings: boolean }) {
			...
			if (plugin.sbcRequired) {
				if (!machinesModelStore[machineName].sbc ||
					(plugin.sbcDsfVersion && !checkVersion(plugin.sbcDsfVersion, machinesModelStore[machineName].sbc.dsf.version)))
				{
