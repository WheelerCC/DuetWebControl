  ...
  return {
    ...
    getters: {
      ...
      hasTemperaturesToDisplay: (state) =>
        (state as MachineModuleState).model.sensors.analog.some(function (sensor, sensorIndex) {
          return (
            (state as MachineModuleState).model.heat.heaters.some(
              (heater) => heater && heater.sensor === sensorIndex,
            ) ||
            (state as MachineModuleState).settings.displayedExtraTemperatures.indexOf(
              sensorIndex,
            ) !== -1
          )
        }),
    ...
    actions: {
      ...
      async upload(
        ...
        const showProgress =
          payload.showProgress !== undefined ? Boolean(payload.showProgress) : true
          ...
          for (let i = 0; i < files.length; i++) {
            ...
            try {
              ...
              const configFile = Path.combine(
                (state as MachineModuleState).model.directories.system,
                Path.configFile,
              )
          ...
          for (let i = 0; i < files.length; i++) {
            ...
            try {
              ...
              if (payload.filename && showSuccess) {
                ...
                log(
                  LogType.success,
                  i18n.t('notification.download.success', [
                    Path.extractFileName(filename),
                    displayTime(secondsPassed),
                  ]),
                  undefined,
              ...
              if (showError && !(e instanceof OperationCancelledError)) {
                ...
                log(
                  LogType.error,
                  i18n.t('notification.download.error', [Path.extractFileName(filename)]),
                  getErrorMessage(e),
                  connector.hostname,
                )
              }
      ...
      async getFileInfo(
        _,
        { filename, readThumbnailContent }: { filename: string; readThumbnailContent?: boolean },
      ...
      async installPlugin(
        ...
        if (plugin.dwcVersion && !checkVersion(plugin.dwcVersion, packageInfo.version)) {
          throw new Error(
            `Plugin ${plugin.id} requires incompatible DWC version (need ${plugin.dwcVersion}, got ${packageInfo.version})`,
          )
        }
        ...
        try {
          ...
          // Install the plugin
          await connector.installPlugin(zipFilename, zipBlob, zipFile, plugin, start)
        } finally {
          // Done
          commit('setMultiFileTransfer', false)
        }
      ...
      async uninstallPlugin(_, plugin: Plugin): Promise<void> {
        if (connector === null) {
          throw new OperationFailedError(
            'uninstallPlugin is not available in default machine module',
          )
        }
      ...
      async setSbcPluginData(
        _,
        { plugin, key, value }: { plugin: string; key: string; value: any },
      ): Promise<void> {
        if (connector === null) {
          throw new OperationFailedError(
            'setSbcPluginData is not available in default machine module',
          )
        }
      ...
      async startSbcPlugin(_, plugin: string): Promise<void> {
        if (connector === null) {
          throw new OperationFailedError(
            'startSbcPlugin is not available in default machine module',
          )
        }
      ...
      async installSystemPackage(
        ...
        if (connector === null) {
          throw new OperationFailedError(
            'installSystemPackage is not available in default machine module',
          )
        }

        const notification = makeFileTransferNotification(
          FileTransferType.systemPackageInstall,
          filename,
          cancellationToken,
        )
        try {
          try {
            await connector.installSystemPackage(
              filename,
              packageData,
              cancellationToken,
              onProgress,
            ...
            makeNotification(
              LogType.success,
              i18n.t('notification.systemPackageInstall.success', [filename]),
            )
          } catch (e) {
            if (!(e instanceof OperationCancelledError)) {
              makeNotification(
                LogType.error,
                i18n.t('notification.systemPackageInstall.error', [filename]),
                getErrorMessage(e),
              )
              throw e
            }
      ...
      async uninstallSystemPackage(_, pkg: string): Promise<void> {
        if (connector === null) {
          throw new OperationFailedError(
            'uninstallSystemPackage is not available in default machine module',
          )
        }
      ...
      async loadDwcPlugin(
        ...
        { id, saveSettings }: { id: string; saveSettings: boolean },
      ) {
        if (connector === null) {
          throw new OperationFailedError('loadDwcPlugin is not available in default machine module')
        }
        ...
        const machineState = state as MachineModuleState,
          plugin = machineState.model.plugins.get(id)
        if (!plugin) {
        ...
        if (plugin.sbcRequired) {
          if (
            !machineState.model.sbc ||
            (plugin.sbcDsfVersion &&
              !checkVersion(plugin.sbcDsfVersion, machineState.model.sbc.dsf.version))
          ) {
            throw new Error(
      ...
      async update({ state, commit }, payload: any) {
        const machineState = state as MachineModuleState
        const lastBeepFrequency = machineState.model.state.beep
          ? machineState.model.state.beep.frequency
          : null
        const lastBeepDuration = machineState.model.state.beep
          ? machineState.model.state.beep.duration
          : null
        const lastDisplayMessage = machineState.model.state.displayMessage,
          lastStatus = machineState.model.state.status
        const lastDsfVersion = machineState.model.sbc?.dsf?.version
        const lastStartupError = machineState.model.state.startupError
          ? JSON.stringify(machineState.model.state.startupError)
          : null
        ...
        if (
          ...
          payload.job.lastFileName !== machineState.model.job.lastFileName
        ) {
        ...
        if (
          lastStatus !== machineState.model.state.status &&
          machineState.model.state.status === MachineStatus.halted
        ) {
          log(
            LogType.warning,
            i18n.t('events.emergencyStop'),
            undefined,
            connector?.hostname ?? DefaultHostname,
          )
        }
      },
      ...
      async onConnectionError({ state, dispatch }, error: Error) {
        ...
        const machineState = state as MachineModuleState
        if (
          !machineState.isReconnecting &&
          [MachineStatus.updating, MachineStatus.halted].includes(machineState.model.state.status)
        ) {
          ...
          await dispatch(
            'onConnectionError',
            { hostname: connector.hostname, error },
            { root: true },
          )
        }
      },
