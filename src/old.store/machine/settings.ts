...
export default function (connector: BaseConnector | null): MachineSettingsModule {
  ...
  return {
    ...
    getters: {
      moveSteps: (state) =>
        function (axis: AxisLetter) {
          return state.moveSteps[axis] !== undefined
            ? state.moveSteps[axis]
            : state.moveSteps.default
        },
      ...
      async save({ state, rootState, dispatch }) {
        ...
        if (rootState.settings.settingsStorageLocal) {
          ...
          try {
            ...
            await dispatch(
              `machines/${connector.hostname}/upload`,
              {
      ...
      async load({ rootState, dispatch, commit }) {
        ...
        if (rootState.settings.settingsStorageLocal) {
          ...
          try {
            settings = await dispatch(
              `machines/${connector.hostname}/download`,
              {
                filename: Path.dwcSettingsFile,
                showProgress: false,
                showSuccess: false,
        ...
        if (mainSettings && mainSettings.enabledPlugins) {
          if (machineSettings && machineSettings.enabledPlugins) {
            /*await*/ dispatch(
              'loadDwcPlugins',
              [...mainSettings.enabledPlugins, ...machineSettings.enabledPlugins],
              { root: true },
            )
          } else {
    ...
    mutations: {
      ...
      toggleExtraVisibility(state, sensor) {
        if (state.displayedExtraTemperatures.indexOf(sensor) === -1) {
          ...
          state.displayedExtraTemperatures = state.displayedExtraTemperatures.filter(
        ...
        if (payload.moveSteps !== undefined) {
          for (let axis in payload.moveSteps) {
            ...
            if (
              ...
              axisMoveSteps.length === state.moveSteps.default.length
            ) {
