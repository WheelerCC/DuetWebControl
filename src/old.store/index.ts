...
const store = new Vuex.Store<InternalRootState>({
  ...
  getters: {
    ...
    isConnected: (state) =>
      ...
      !(state as RootState).machines[state.selectedMachine].isReconnecting,
  ...
  actions: {
    ...
    async connect(
      ...
      try {
        ...
        if (
          (state as RootState).settings.lastHostname !== location.host ||
          hostname !== location.host
        ) {
        ...
        if (isPasswordError) {
          ...
          setTimeout(
            () => dispatch('connect', { hostname, username, password, retrying: true }),
            1000,
          )
          return
        }
    ...
    async onConnectionError(
      { dispatch, commit },
      { hostname, error }: { hostname: string; error: Error },
    ) {
