<template>
  ...
  >
    <v-card class="d-flex flex-column">
      ...
      <div v-if="useMonacoEditor" ref="monacoEditor" class="editor-monaco" />
      <v-textarea
        v-else
        ref="textarea"
        hide-details
...
export default defineComponent({
  ...
  computed: {
    fffMode(): boolean {
      return useMachinesModelStore().state.machineMode === MachineMode.fff
    },
    ...
    useMonacoEditor(): boolean {
      return !useRootStore().oskEnabled && !this.isMobile
    },
    language(): string {
      if (
  ...
  methods: {
    ...
    onTextareaTab(e: Event) {
      ...
      const textStart = textArea.value.slice(0, originalSelectionStart),
        textEnd = textArea.value.slice(originalSelectionStart)
