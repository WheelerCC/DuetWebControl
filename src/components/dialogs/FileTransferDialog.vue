      ...
      <v-card-actions>
        ...
        <v-btn v-show="canCancel" color="blue darken-1" text @click="cancel">
          {{
            $t(
              isUploading
                ? 'dialog.fileTransfer.cancelUploads'
                : 'dialog.fileTransfer.cancelDownloads',
            )
          }}
        </v-btn>
        <v-btn
...
export default defineComponent({
  ...
  computed: {
    ...
    title(): string {
      ...
      const translation = this.isUploading
        ? 'dialog.fileTransfer.uploadingTitle'
        : 'dialog.fileTransfer.downloadingTitle'
      return this.$t(translation, [
        fileBeingTransferred,
        this.files.length,
    ...
    canCancel(): boolean {
      return (
        !this.transfersFinished &&
        this.cancellationTokens[useRootStore().selectedMachine] !== undefined
      )
    },
    ...
    transfersFinished(): boolean {
      ...
      return !this.files.some(
        (file) => file.startTime === null || file.progress === null || file.progress < 1,
      )
    },
  },
  mounted() {
  ...
  methods: {
    ...
    fileComplete({ machine, num, count }: { machine: string; num: number; count: number }) {
      if (
        useRootStore().selectedMachine === machine &&
        num + 1 === count &&
        this.closeProgressOnSuccess[machine]
      ) {
        this.close()
        ...
        if (fileTable.rows.length > num + 1) {
          ...
          fileTable.rows[fileTable.rows.length - 1].scrollIntoView({
            behavior: 'smooth',
            block: 'center',
          })
        }
