        ...
        >
          <td v-for="header in props.headers" :key="header.value" :class="header.cellClass">
            ...
            <template v-else-if="header.unit === 'bytes'">
              {{
                props.item[header.value] !== null && !props.item.isDirectory
                  ? $displaySize(props.item[header.value])
                  : ''
              }}
            </template>
            <template v-else-if="header.unit === 'date'">
              {{
                props.item.lastModified
                  ? props.item.lastModified.toLocaleString()
                  : $t('generic.noValue')
              }}
            </template>
    ...
    <confirm-dialog
      ...
      :prompt="
        removeDialog.items.length > 1
          ? $t('dialog.deleteFiles.deleteMulitiplePrompt')
          : $t('dialog.deleteFiles.deletePrompt', [
              removeDialog.items.length > 0 ? removeDialog.items[0].name : '',
            ])
      "
...
export default VDataTable.extend({
  ...
  computed: {
    ...
    transferringFiles(): boolean {
      return useMachinesStore().transferringFiles
    },
    ...
    isLoading(): boolean {
      return (
        this.loading || this.innerLoading || this.doingFileOperation || this.innerDoingFileOperation
      )
    },
    ...
    noItemsText(): string {
      return this.innerFilelistLoaded ||
        this.isMounted ||
        useRootStore().selectedMachine === defaultMachine
        ? this.noFilesText
        : 'list.baseFileList.driveUnmounted'
    },
    internalSortBy: {
      get(): string {
        return useMachinesCacheStore().sorting[this.sortTable!].column
      },
      set(value: string) {
    ...
    internalSortDesc: {
      get(): boolean {
        return useMachinesCacheStore().sorting[this.sortTable!].descending
      },
      set(value: boolean) {
        useMachinesCacheStore().setSorting({
          table: this.sortTable!,
  ...
  methods: {
    ...
    async loadDirectory(directory: string) {
      ...
      if (
        ...
        this.innerDirectory =
          volume === Path.getVolume(this.initialDirectory) ? this.initialDirectory : `${volume}:`
        this.innerFilelist = []
    ...
    onItemDragStart(item: BaseFileListItem, e: DragEvent) {
      if (
        this.noDragDrop ||
        this.contextMenu.touchTimer ||
        this.contextMenu.shown ||
        e.target === null ||
        e.dataTransfer === null
      ) {
        return
      ...
      Array.from(tableClone.tBodies[0].rows).forEach(function (row) {
        ...
        if (itemsToDrag.some((item) => (item.isDirectory ? '*' : '') + item.name === filename)) {
          Array.from(row.children).forEach((td, index) => {
            if (td.tagName === 'TD') {
              ;(td as HTMLTableCellElement).style.width =
                `${(firstRow.children[index] as HTMLTableCellElement).offsetWidth}px`
            } else {
              td.remove()
            }
          })
      ...
      if (!this.noDragDrop && item.isDirectory && e.dataTransfer !== null) {
        ...
        if (jsonData) {
          ...
          if (
            ...
            !data.items.some((dataItem) => dataItem.isDirectory && dataItem.name === item.name)
          ) {
            e.preventDefault()
            e.stopPropagation()
          }
    ...
    async removeCallback() {
      ...
      const deletedItems: any[] = [],
        directory = this.directory // todo correctly type
      for (const item of this.removeDialog.items) {
        try {
          ...
          this.innerFilelist = this.innerFilelist.filter(
      ...
      if (deletedItems.length) {
        this.$log(
          ...
          deletedItems.length > 1
            ...
            : this.$t('notification.delete.success', [deletedItems[0].name]),
        )
      }
      this.innerDoingFileOperation = false
    },
    ...
    filesOrDirectoriesChanged({
      ...
      if (
        ...
        ((files !== undefined && Path.filesAffectDirectory(files, this.directory)) ||
          volume === Path.getVolume(this.directory))
      ) {
