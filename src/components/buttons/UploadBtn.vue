...
export default Vue.extend({
      ...
      switch (this.target) {
        ...
        case UploadType.system:
          return (
            ...
            (useMachinesModelStore().sbc !== null ? ',.deb' : '')
          )
        case UploadType.web:
  ...
  methods: {
    ...
    async doUpload(
      ...
      if (!zipName) {
        ...
        if (
          ...
          files[0].name.toLowerCase() === 'dsf-update.zip'
        ) {
          ...
          if (Path.isSdPath('/' + content.name)) {
            ...
            if (firmwareFileName) {
              filename = Path.combine(
                useMachinesModelStore().directories.firmware,
                firmwareFileName,
              )
    ...
    async startUpdate() {
      ...
      this.confirmReset =
        !modules.includes(0) && this.updates.firmwareBoards.some((board) => board > 0)
    },
    async waitForUpdate() {
      do {
