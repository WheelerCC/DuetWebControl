  ...
  <v-card>
    ...
    <v-card-text class="pa-0 img-container">
      <v-responsive v-if="webcam.embedded" :aspect-ratio="16 / 9">
        <iframe :src="webcam.url" :class="classList" />
      </v-responsive>
...
const webcamIsRTC = computed(
  () => webcam.value.url.startsWith('ws:') || webcam.value.url.startsWith('wss:'),
)
...
async function connectWebRTC() {
  ...
  if (remoteVideo.value !== null) {
    const audioTrack = peerConnection!.addTransceiver('audio', { direction: 'recvonly' }).receiver
      .track
    const videoTrack = peerConnection!.addTransceiver('video', { direction: 'recvonly' }).receiver
      .track
...
function update() {
  let urlValue = webcam.value.url.replace(
    '[HOSTNAME]',
    store.getters['machine/connector']
      ? store.getters['machine/connector'].hostname
      : location.hostname,
  )
