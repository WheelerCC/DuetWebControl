<template>
  <v-card>
    ...
    <v-card-text v-if="hasVisibleFans" class="d-flex flex-column pb-0">
      <div v-if="displayedFans.includes(-1) && toolFanValue >= 0" class="d-flex flex-column pt-2">
        {{ $t('panel.fans.toolFan') }}
        <percentage-input
          :value="toolFanValue"
          :disabled="uiFrozen"
      ...
      <template v-for="(fan, index) in fans">
        <div
          v-if="
            displayedFans.includes(index) && fan !== null && fan.thermostatic.sensors.length === 0
          "
          :key="index"
...
export default Vue.extend({
  computed: {
    ...
    displayedFans(): Array<number> {
      return useMachinesSettingsStore().displayedFans
    },
    fans(): Array<Fan | null> {
    ...
    hasVisibleFans(): boolean {
      if (
        this.fans.some(
          (fan, index) =>
            this.displayedFans.includes(index) &&
            fan !== null &&
      ...
      return (
        this.displayedFans.includes(-1) &&
        this.toolFan >= 0 &&
        this.toolFan < this.fans.length &&
        this.fans[this.toolFan] !== null &&
        this.fans[this.toolFan]!.thermostatic.sensors.length === 0
      )
    },
    toolFan(): number {
  ...
  methods: {
    isFanVisible(fanIndex: number) {
      ...
      return (
        fanIndex < this.fans.length &&
        this.fans[fanIndex] !== null &&
        this.fans[fanIndex]!.thermostatic.sensors.length === 0
      )
    },
    getFanValue(fanIndex: number) {
      ...
      return fanIndex >= 0 && fanIndex < this.fans.length && this.fans[fanIndex] !== null
        ? Math.round(this.fans[fanIndex]!.requestedValue * 100)
        : 0
    },
