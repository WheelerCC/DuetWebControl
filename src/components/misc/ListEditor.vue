...
export default defineComponent({
  ...
  computed: {
    items(): Array<number> | { active: Array<number>; standby: Array<number> } {
      return this.itemKey === 'spindleRPM'
        ? useMachinesSettingsStore().spindleRPM
        : useMachinesSettingsStore().temperatures[this.itemKey]
    },
    unit(): string {
      return this.temperature ? '°C' : 'RPM'
    },

    canAddActive(): boolean {
      return (
        isFinite(this.activeValue) &&
        !(this.items instanceof Array) &&
        !this.items.active.includes(this.activeValue)
      )
    },
    canAddStandby(): boolean {
      return (
        isFinite(this.standbyValue) &&
        !(this.items instanceof Array) &&
        !this.items.standby.includes(this.standbyValue)
      )
    },
    canAdd(): boolean {
      return isFinite(this.value) && this.items instanceof Array && !this.items.includes(this.value)
    },
  },
  methods: {
