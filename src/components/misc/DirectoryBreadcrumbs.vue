...
export default defineComponent({
  ...
  computed: {
    directories(): Directories {
      return useMachinesModelStore().directories
    },
    pathItems(): Array<BreadcrumbItem> {
      ...
      showDropdown =
        showDropdown &&
        pathItems.length === 1 &&
        (this.hasDirectDisplay || this.firmwareDirectoryDiffers)
      ...
      for (const item of pathItems) {
        ...
        if (index === 0) {
          items.push({
            showDropdown,
            text: item.startsWith('0:')
              ? rootCaption
              : this.$t('generic.sdCard', [/^(\d+)/.exec(item)![1]]),
    ...
    dragOver(directory: string, e: DragEvent) {
      ...
      if (jsonData) {
        ...
        if (
          ...
          !data.items.some(
            (dataItem) =>
              dataItem.isDirectory && directory === Path.combine(data.directory, dataItem.name),
          )
        ) {
          e.preventDefault()
          e.stopPropagation()
