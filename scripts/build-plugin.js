...
if (pluginDir.startsWith(dwcPluginDir)) {
  ...
  const srcDirectory = fs.existsSync(pluginDir + '/dwc-src')
    ? pluginDir + '/dwc-src'
    : pluginDir + '/src'
  try {
...
if (fs.existsSync(`${dwcPluginDir}/${pluginManifest.id}/dwc-src/index.js`)) {
  ...
  fs.renameSync(
    `${dwcPluginDir}/${pluginManifest.id}/index.ts`,
    `${dwcPluginDir}/${pluginManifest.id}/index-ts.ts`,
  )
  ...
  .then(() => {
    ...
    if (fs.existsSync(distDir)) {
      ...
      if (pluginManifest.version === 'auto') {
        archive.pipe(
          fs.createWriteStream(distDir + `/${pluginManifest.id}-${dwcManifest.version}.zip`),
        )
      } else {
        archive.pipe(
          fs.createWriteStream(distDir + `/${pluginManifest.id}-${pluginManifest.version}.zip`),
        )
      }
      ...
      // Add plugin JSON
      if (
        pluginManifest.version === 'auto' ||
        pluginManifest.dwcVersion === 'auto' ||
        pluginManifest.dwcVersion === 'auto-major' ||
        pluginManifest.sbcDsfVersion === 'auto' ||
        pluginManifest.sbcDsfVersion === 'auto-major' ||
        pluginManifest.rrfVersion === 'auto' ||
        pluginManifest.rrfVersion === 'auto-major'
      ) {
        if (pluginManifest.version === 'auto') {
      ...
      if (pluginManifest.dwcVersion && !filesAdded) {
        error(
          'No Webpack chunks have be added to the ZIP archive. Make sure it is imported by src/plugins/index.ts',
        )
        process.exit(1)
      }
